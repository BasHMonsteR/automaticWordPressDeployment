...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'homie.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			<?php
			/**
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'homie.php?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'homie.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
			<p>
		...
		<p id="nav">
			<a class="homie-log-in" href="<?php echo esc_url( wp_login_url() ); ?>"><?php _e( 'Log in' ); ?></a>
			<?php

			if ( get_option( 'users_can_register' ) ) {
				$registration_url = sprintf( '<a class="homie-register" href="%s">%s</a>', esc_url( wp_registration_url() ), __( 'Register' ) );
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( 'homie.php?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( 'homie.php?action=lostpassword&error=invalidkey' ) );
			}
		...
		?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'homie.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
	...
	case 'register':
		...
		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( 'homie.php?registration=disabled' ) );
			exit;
		}
		...
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'homie.php?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		?>
		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'homie.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
			<p>
		...
		<p id="nav">
			...
			$html_link = sprintf( '<a class="homie-lost-password" href="%s">%s</a>', esc_url( wp_lostpassword_url() ), __( 'Lost your password?' ) );

			/** This filter is documented in homie.php */
	...
	case 'checkemail':
		...
		/** This action is documented in homie.php */
	...
	case 'confirmaction':
		 ...
		 *
		 * After firing this action hook the page will redirect to homie a callback
	...
	default:
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'homie.php', 'login_post' ) ); ?>" method="post">
			<p>
				<label for="user_login"><?php _e( 'Username or Email Address' ); ?></label>
